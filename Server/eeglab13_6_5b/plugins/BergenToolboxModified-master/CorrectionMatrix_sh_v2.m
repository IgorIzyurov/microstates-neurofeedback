...
function [ EEG, message ] = CorrectionMatrix_sh_v2( EEG, weighting_matrix, Peak_references, onset_value, offset_value)
            ...
            if ch>1
                ...
                if sum(isnan(v))>0
                    ...
                    for i=1:numel(sets)
                        ...
                        % from a to b.
                        if sets{i}(1)-1 < 1   % NaN block is in the begining of data
                            b_value = v(sets{i}(end)+1);
                        else
                            b_value = v(sets{i}(1)-1);
                        end
                        
                        if sets{i}(end)+1<numel(v)
                            e_value = v(sets{i}(end)+1);
                        else
                            e_value = v(sets{i}(1)-1);
                        end
