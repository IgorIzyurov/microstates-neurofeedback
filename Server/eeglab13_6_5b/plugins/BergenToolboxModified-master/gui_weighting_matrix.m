...
%
% Last Modified on 12-Apr-2010 10:30:11
%
% Improve performance to plot final results with large data
% Bug fix - reading single channel when importing dataset files in .M
% format.
...
%Protejer o campo e verificar que � colocado um valor superior a 100ms
...
set(gca, 'XTick', [0 pace pace*2 pace*3 pace*4 pace*5 pace*6 pace*7]);
set(gca,'XTickLabel', x/EEG.srate*1000);
ylabel('Gradient of EEG data in [�V]:');
xlabel('Time in [ms]');
hold off;
        ...
        if complete == 3
            try
                if (n_fMRI - fmri_weighting_matrix ) >= 0;
                    ...
                    % maybe annotate HERE (!!) where troubles occurred (put it
                    % in the EEG trace); hopefully it'll be taken care of.
                    EEG.annotations.badtemplate=false(size(EEG.data));
                    % keyboard;
                    
                    % beware of THESE:
                    annotate_these_volumes = find(motiondata_struct.both>0);
                    for anno_i = 1:numel(annotate_these_volumes)
                        anno_begin =  Peak_references(1) + (annotate_these_volumes(anno_i)-1)*mean(diff(Peak_references(:)));
                        anno_end = anno_begin + round(mean(diff(Peak_references(:)))) - 1;
                        EEG.annotations.badtemplate(:,anno_begin:anno_end)=true;
                    end
                    
                    % annotate also (while you're at it..) the stuff BEFORE the
                    % artifact correction begins, and also AFTER the artifact
                    % corrections stops!
                    EEG.annotations.uncorrected=false(size(EEG.data));
                    
                    
                    % the beginning of the recording...
                    the_begin = Peak_references(1);
                    %if the_begin>1
                    %    the_begin=the_begin - 1;
                    %end
                    
                    % the end of the recording...
                    the_end = Peak_references(end)+round(mean(diff(Peak_references(:))));
                    if the_end > EEG.pnts
                        error('you need to remove the last MRI-scanner-marker;\nThere\''s not enough data left in the trace for an entire MRI volume');
                        %elseif the_end < EEG.pnts
                        %    the_end = the_end+1;
                    end
                    EEG.annotations.uncorrected(:,1:the_begin)=true;
                    EEG.annotations.uncorrected(:,the_end+1:end)=true;
                    % keyboard;
...
log_string = [log_string, 'Gradient Threshold [�V]: ', get(handles.grad_txt,'string'),10];
...
switch baseline_method
    ...
    otherwise
        ...
        if baseline_method == 1
            ...
            log_string = [log_string, 'Baseline amplitude value [�V]: 0',10];
        end
        ...
        if baseline_method == 2
            ...
            log_string = [log_string, 'Baseline amplitude value [�V]: Variable',10];
        end
...
%Step 6
set(handles.n_channels_txt,'String',num2str(EEG.nbchan));
        ...
        try
            ...
            if error == 1
                ...
                if get(handles.apply_filter_checkbox,'value') == 1
                    ...
                    EEG = pop_eegfilt( EEG, 0, high_edge, [], [0]);
                    ...
                    for i_ch=1:numel(EEG.chanlocs)
                        if sum(strcmp(EEG.chanlocs(i_ch).labels,{'CW 1','CW 2','CW 3','CW 4','CW 5','CW 6','CW 7','CW 8'}))>0
                            % keyboard;
                            if (sum(EEG.annotations.nansamples(i_ch,:))/numel(EEG.annotations.nansamples(i_ch,:)))>0.005;
                                do_additional_channels(end+1) = i_ch;
                                disp(['doing extra-low for: ' EEG.chanlocs(i_ch).labels]);
                            end
                        end
                    end
                    % but... this should really go into the CW Regression Toolbox!!
                    disp('CW Regression -->');
                    disp(do_additional_channels);
                    do_additional_channels=[];
		    ...
		    check_ecg = [];
                    for i_ecg=1:numel(check_ecg)
                        newvec=eegfilt(EEG.data(check_ecg(i_ecg),:),EEG.srate,0,100);
                        EEG.data(check_ecg(i_ecg),:)=newvec;
                        fprintf('did 100 Hz low-pass on ecg channel %s.\n',EEG.chanlocs(check_ecg(i_ecg)).labels);
                    end
                    close(h);
                    fprintf(['Done!',10]);
                end
            ...
            % Run Resampling (changed orders... maybe be slower but I think it's a bit
            % safer. otherwise it'll be filtered twice??
            if error == 1
            else
                if get(handles.resample_checkbox,'value') == 1
                    fprintf(['Resampling...']);
                    ...
                    % keyboard;
                    % so... resampling my boolean annotation matrices.
                    % first inspect them.
                    fprintf(['Done!',10]);
                end
            end
            
            % Run high-pass filtering (changed orders... maybe be slower but I think it's a bit
            % safer. otherwise it'll be filtered twice??
            ...
            if error == 1
                ...
                if get(handles.apply_filter_checkbox,'value') == 1
                    ...
                    if low_edge > 0
                        ...
                        fprintf(['High-pass filter applied.',10]);
            ...
            % now... downsample my annotation vectors!!
            if error == 1
            else
                
                % keyboard;
                resample_factor = str2num(get(handles.resample_rate_txt,'string'));
                downsample_by = old_samplingrate / resample_factor;
                
                fprintf('updating annotation matrices... downsampling them by %d:\n',downsample_by);
                
                annotation_fields = fieldnames(EEG.annotations);
                % annotation_fields = {'badcorrelation','badtemplate','nansamples','uncorrected'};
                try
                    for i_annotation_fields = 1:numel(annotation_fields);
                        
                        EEG.annotations.(annotation_fields{i_annotation_fields})=downsample(EEG.annotations.(annotation_fields{i_annotation_fields})',downsample_by)';
                        if size(EEG.annotations.(annotation_fields{i_annotation_fields}),2)<size(EEG.data,2);
                            diff_in_size = size(EEG.data,2) - size(EEG.annotations.(annotation_fields{i_annotation_fields}),2);
                            EEG.annotations.(annotation_fields{i_annotation_fields}) = [EEG.annotations.(annotation_fields{i_annotation_fields}) false(size(EEG.data,1),diff_in_size)];
                        end
                    end
                catch
                    % keyboard;
                    disp('something occurred at lines: 2563 in gui_weighting_matrix');
                    disp('has to do with updating/downsampling my annotations!');
...
if EEG.srate >= 1000 && EEG.srate < 10000
    reslice=10;
end
if EEG.srate >= 10000
    reslice=100;
end
plot(EEG.data(ch,1:reslice:end));
x =  get(gca, 'XTick');
set(gca,'XTickLabel', x/EEG.srate*1000*reslice);
ylabel('Threshold in [�V]:');
xlabel('Time in [ms]');
