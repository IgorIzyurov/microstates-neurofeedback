function [b_model,GEV] = eeg_computeAAHC_v3_Marina(eeg,n_mod,ProgBar, IgnorePolarity)
...
%     Cluster = NormDim(eeg,2) / sqrt(n_chan);
    Cluster = NormDim(eeg,2) / sqrt(n_chan);
%     Cluster = eeg;
    ...
    while (nClusters) > min(n_mod)
        ...
        for f = 1:numel(ClusterToUpdate)  % Some cluster center need an update
            ...
            if (IgnorePolarity == true);        
                ...
                Cluster(ClusterToUpdate(f),:) = NormDim(pc1',2) / sqrt(n_chan);
            else
                Cluster(ClusterToUpdate(f),:) = NormDim(mean(eeg(ClusterMembers,:),1),2) / sqrt(n_chan);
            end
