...
%
function [MSClass, gfp, gfpsmooth, fit] = AssignMStates_v2(eegdata, Maps, params, IgnorePolarity, chanlocs)
    ...
    gfpsmooth = nan(1,size(eegdata.data,2),size(eegdata.data,3));
    ...
    if params.PeakFit == 1
        ...
        gfpsmoothed = conv(gfp(:,:,s), gaussFilter, 'same');
        IsIn = find([false (gfpsmoothed(1,1:end-2) < gfpsmoothed(1,2:end-1) & gfpsmoothed(1,2:end-1) > gfpsmoothed(1,3:end)) false]);
        gfpsmooth(:,:,s) = gfpsmoothed;
            if isempty(IsIn)
