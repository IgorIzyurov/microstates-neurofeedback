function [MeanMap,SortedMaps,OldMapFit,Maplimits] = PermutedMeanMaps_v4(in,RespectPolarity, debug, Montage, Conditon, Maplimits, EEG_chanlocs, mycmap)
...
%         for counter = 1:ceil(nSubjects/MaxSubjPerPicture)
        for counter = 1:1
            ...
            saveas(gcf, sprintf('C:\\Data\\Neurim\\EEG\\Microstates_V01\\test1Iteration%.jpg', ClustPar.UseAAHC, subject_index, day, session_num))
        end
...
%             for c = 1:nMaps
%                 subplot(MaxSubjPerPicture+1,nMaps,spidx);
%                 topoplot(MeanMap(c,:),EEG_chanlocs,'maplimits',[-Maplimits,Maplimits],'style','fill','electrodes','off');
%                 spidx = spidx+1;
%                 title('mean maps','FontSize',6);
%             end
%             colormap(mycmap)
