...
if isequal(Condition,'Upreg') || isequal(Condition,'Downreg')
    ...
    switch Condition
        case 'Upreg'
            MaxColor = [0.4844 0.9875 0];
        case 'Downreg'
            MaxColor = [0.8594 0.0781 0.2344];
    end
    
    PreviousColor = options.PreviousColor;
    PreviousColor = [0.5 0.5 0.5];
    CurrentFB_pix = (options.MaxFBScreen * CurrentFB)/(options.TrialLengthSeconds*(1/2));
    PreviousFB_pix = (options.MaxFBScreen * PreviousFB)/(options.TrialLengthSeconds*(1/2));
    MaximalFB_pix = (options.MaxFBScreen * MaximalFB)/(options.TrialLengthSeconds*(1/2));
