...
%
function [MSClass,gfp, fit] = AssignMStates_Igor(TheEEGData, sRate, Maps, params, IgnorePolarity)
    ...
    if params.PeakFit == 1
        ...
        for s = 1:nSegments
            ...
            if isempty(IsIn)
                ...
                MSClass = [];
                continue
            elseif size(IsIn,2) == 1
                errordlg2('1 GFP peaks found','Microstate fitting');
                %MSClass = [];
                continue
            end
        ...
        MSClass = zeros(size(TheEEGData,2),nSegments);
        for s = 1:nSegments
            [Winner,ExpVar] = SmoothLabels(TheEEGData(:,:,s),Maps,params, sRate,IgnorePolarity);
            % Kill microstates truncated by boundaries
            if params.BControl == true
                for b = 1:numel(BoundaryPoint)
                    if BoundaryEpoch(b) == s
