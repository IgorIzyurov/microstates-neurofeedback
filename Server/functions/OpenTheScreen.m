...
if windowOpts.windowed == 1
    windowOpts.screen_num = 2;
    screen_rect = Screen('Rect',0);
    [mx,my] = RectCenter(screen_rect);
    windowOpts.window_width = 800;
    windowOpts.window_height = 600;
...
elseif windowOpts.windowed == 0
    if SkipSync
        ...
        windowOpts.screen_num = max(screens);
        windowOpts.screen_num = 2; % On Igor's computer
    else
        windowOpts.screen_num = 1; % On Igor's computer
    end
    windowOpts.window_size = []; % Full screen
end

screen.Color = BlackIndex(windowOpts.screen_num); % Define black and white
[window, rect] = PsychImaging('OpenWindow', windowOpts.screen_num, screen.Color); % Open an on screen window
