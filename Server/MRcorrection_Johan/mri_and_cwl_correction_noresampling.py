...
bpf=BPF(f=[1.0, 20.0], fs=fs, order=3, nbchan=nbchan)
if wantResample == 1:
    cwl=CWL(seconds_in_window=6.0, tdelay=0.050, ichs=EEG_channels, icws=CWL_channels, fs=fs_resampled, highpass=[], saveglms=False)
else:
    cwl=CWL(seconds_in_window=6.0, tdelay=0.050, ichs=EEG_channels, icws=CWL_channels, fs=fs, highpass=[], saveglms=False)
...
if wantResample == 1:
    ...
    infoCWL = StreamInfo('EEG-MR-CWL', 'EEG-MR-CWL', nbchan, fs, 'double64', 'corrected')
    infoMR = StreamInfo('EEG-MR', 'EEG-MR', nbchan, fs, 'double64', 'uncorrected')
