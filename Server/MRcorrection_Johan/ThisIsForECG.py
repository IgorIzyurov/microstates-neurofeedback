...
hpf=HPF(f=0.1, fs=fs, order=3, nbchan=nbchan)
lpf=LPF(f=10, fs=fs, order=3, nbchan=nbchan)
...
infoCWL = StreamInfo('ECG-fs-1000', 'EEG-MR-CWL', nbchan, fs_resampled, 'double64', 'corrected')
infoMR = StreamInfo('ECG-fs-5000', 'EEG-MR', nbchan, fs, 'double64', 'uncorrected')
...
print('acquisition started. Resampling to 500 Hz. Bandpassing to 1-125 Hz')
while True:
    data, marker = amp.get_data()
    # it doesn't make sense to do stuff, if theere is no data
    if data.shape[0] > 0:
        data2=hpf.handle(data)
        data3=lpf.handle(data2)
        data4=resample.handle(data3)
        ...
        # pdb.set_trace()
        outletCWL.push_chunk(data4.tolist())
        outletMR.push_chunk(data3.tolist())
